...
class UsersController extends AppController {
	...
	// If data isn't empty, try to authenticate user using OpenID.
        if (!empty($this->data)) {
            try {
                $this->Openid->authenticate($this->data['OpenidUrl']['openid'], $returnTo, 'http://'.Configure::read('ip').Configure::read('relativeUrl'));
            } catch (InvalidArgumentException $e) {
                $this->Session->setFlash('Invalid OpenID', 'default', array('class' => 'error'));
            } catch (Exception $e) {
